...
| `--client claude-code` / `--client claude` | No config file written; runs `claude mcp add --scope user --transport stdio pionex-trade-mcp -- @pionex/pionex-trade-mcp` |
| `--client windsurf` | `~/.codeium/windsurf/mcp_config.json` |
| `--client vscode` | `.mcp.json` in current directory |

Then **restart your client** (Cursor, Claude Desktop, Claude Code, etc.).
...
- ✅ Correct:
  ...
  ```bash
  mcporter call pionex pionex_orders_new_order \
...
- ❌ Incorrect (wrapping under `schema` can break parsing):
  ...
  ```bash
  mcporter call 'pionex.pionex_orders_new_order(schema:{...})'
  ```
