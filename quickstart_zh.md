...
| `--client claude-code` / `--client claude` | 不写入配置文件；会执行 `claude mcp add --scope user --transport stdio pionex-trade-mcp -- @pionex/pionex-trade-mcp` |
| `--client windsurf` | `~/.codeium/windsurf/mcp_config.json` |
| `--client vscode` | 当前目录下的 `.mcp.json` |

完成后 **重启 Cursor、Claude Desktop、Claude Code 或对应客户端**。
