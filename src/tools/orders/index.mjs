...
/**
 * Register order tools (require auth).
...
export function registerOrdersTools(server) {
  server.tool(
    ...
    "Create a spot order on Pionex. " +
      ...
      "Common error codes from the exchange include: " +
      "`TRADE_AMOUNT_FILTER_DENIED` (amount too small/large or invalid), " +
      "`TRADE_SIZE_FILTER_DENIED` (size below minimum or wrong step), " +
      "`TRADE_NOT_ENOUGH_MONEY` (insufficient balance), and " +
      "`TRADE_INVALID_SYMBOL` (symbol is not a valid trading pair, e.g. use ADA_USDT).",
    {
      schema: z.object({
